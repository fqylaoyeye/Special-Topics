...
global n m A B r_total r_count lambda con;
n = 100;        %process
m = 15;         %good
con = -0.002;
lambda = 0.5;
...
%       
A = abs(100 * randn(n,m) + 1)+1e-5;                  %consumption function
B = abs(100 * randn(n,m) + 1)+1e-5;                  %production function
%production function
...
%         q0 = q0/sqrt (sum(q0.^2));                 %normalize q
    ...
    r_total(:,(t+1)+1) = (r_total(:,t+1) .* ((repmat(lambda * con,100,1) + B*(q_equi.^2)) ./ (repmat(con,100,1) + A*(q_equi.^2)))) ; 
...
function [E, grad_E] = ExcessDemand(q)
    global m A B r_total r_count con lambda;
