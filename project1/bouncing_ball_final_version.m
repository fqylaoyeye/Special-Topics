...
if(video)
    writerObj = VideoWriter('../videos/Ball_and_meshgrid.mp4','MPEG-4');
...
Eb_save = zeros(clockmax,1);
Em_save = zeros(clockmax,1);
Ek = zeros(clockmax,1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for k=1:kmax_m
  U(k,:) = U(k,:) + dt*F(k,:)./[m,m,m]; 
end
  U(kmax,:)=U(kmax,:) + dt*F(kmax,:)./[M_b,M_b,M_b];
  %update velocities of all points,
  ...
  for l = l_mb
      Eps(clock) = Eps(clock) + 1/2* sum(S(l) .*(R(l)-R0(l)) .^2);
  end

  for k = 1:kmax_m
      Em_save(clock) = Em_save(clock) + 1/2*M(k)*(U(k,1)^2+U(k,2)^2+U(k,3)^2)...
       + M(k)*g*X(k,3);
  end
  for l = 1:lmax_m
      Em_save(clock) = Em_save(clock) + 1/2* S(l) *(R(l)-R0(l)) ^2;
  end
  Eb_save(clock) = Ek(clock)+Epg(clock)+Eps(clock);
