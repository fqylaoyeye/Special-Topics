...
R0_m = 0.5*ones(lmax_m,1);                % rest length
...
figure(1);
fig = figure(1);
    phb=plot3(X_b(1,1),X_b(1,2),X_b(1,3),".","Markersize",32,"Color","r");  % plot the ball
    hold on
    phm=plot3(X(pm,1),X(pm,2),X(pm,3),"O","Markersize",4,"Color","b");         % plot points of the meshgrid
...
clf(fig)
    ...
    phb=plot3(X(kmax,1),X(kmax,2),X(kmax,3),".","Markersize",32,"Color","r");  % plot the ball
    hold on
    phm=plot3(X(pm,1),X(pm,2),X(pm,3),"O","Markersize",4,"Color","b");         % plot points of the meshgrid
%    lhm=plot3([X(jj(lm),1),X(kk(lm),1)]',[X(jj(lm),2),X(kk(lm),2)]',...
%        [X(jj(lm),3),X(kk(lm),3)]',"Color","b","LineWidth",1);            % plot links on the meshgrid
